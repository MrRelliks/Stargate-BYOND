...
obj/Pegasus_Gates/proc
	Connect(obj/Pegasus_Gates/Target)
		...
		spawn()Idle(Target)
		if(Target.name == "Atlantis")
			switch(input("The iris is closed, Press confirm to send IDC, make sure there are no enemies nearby.")in list("Confirm","Abort"))
				if("Confirm")
					if(usr.race == "Tau'ri" && Target.name == "Atlantis")
						usr<<sound('Sounds/iriscode.ogg',volume=10)
						usr << "Sending IDC"
						sleep(5)
						Target.iris.open()
						usr << "Iris opened - It is safe to step through! (iris will automatically close in 10 seconds)"
						sleep(100)
						Target.iris.close()
						usr << "Atlantis Iris closed"
					else
						sleep(30)
						usr << "Iris code unrecognised. Iris remains open."
				if("Abort")
					return
	...
	Disengage()
		for(var/obj/ChevronsSGU/Z in chevrons)Z.Darken()
...
//							usr.UnFadeScreen()
		...
		New()
			..()
			if(chev)overlays+=image('Icons/Stargate/chevrons.dmi',dir,icon_state = "Locking")
			x+=ax
			y+=ay
	ChevronsSGU
		layer=10
